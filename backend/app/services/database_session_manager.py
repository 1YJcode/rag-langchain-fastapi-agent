...
from backend.app.db.db_config import AsyncSessionLocal
from backend.app.models.chat_history import ChatSession, ChatMessage
...
class DatabaseSessionManager:
    ...
    async def add_message(self, session_id, user_id: str, user_message: str, assistant_message: str):
        """添加消息并保存到数据库"""
        async with AsyncSessionLocal() as db:
            # 检查会话id是否存在
            existing_session = await db.run_sync(
            ...
            if existing_session:
                ...
                if str(existing_session.user_id) != str(user_id):
                    ...
                    logger.warning(f"  existing_session.user_id={existing_session.user_id!r}({type(existing_session.user_id).__name__}), user_id param={user_id!r}({type(user_id).__name__})")
                    from fastapi import HTTPException, status
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
    ...
    async def get_all_session_ids(self, user_id: Optional[str] = None) -> List[str]:
        """获取所有会话ID，如果提供了user_id，则只返回该用户的会话"""
        async with AsyncSessionLocal() as db:
            if user_id:
    ...
    async def get_user_session(self, user_id: str) -> List[Dict]:
        """获取用户所有会话详细信息"""
        async with AsyncSessionLocal() as db:
            sessions = await db.run_sync(
                lambda session: session.query(ChatSession).filter(ChatSession.user_id == user_id).order_by(ChatSession.updated_at.desc()).all()
            )
            return [
                {
                    "id": session.id,
