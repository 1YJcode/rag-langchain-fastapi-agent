...
def log_before_model(status: AgentState, runtime: Runtime):
    """model 执行前执行此函数"""
    logger.info(f"[before_model] model启动，输入：{status['messages']}，共{len(status['messages'])}条信息")

@after_model
def log_after_model(status: AgentState, runtime: Runtime):
    """model 运行后执行此函数"""
    logger.info(f"[after_model] model运行结束，输出：{status['messages']}，共{len(status['messages'])}条信息")
