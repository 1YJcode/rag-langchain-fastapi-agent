...
class AsyncTextSplitter:
    ...
    def __init__(self,
        ...
        """
        # 默认分割符，优先级排序
        default_separators = chroma_config['default_separators']
        ...
        self.merge_max_ratio = merge_max_ratio
    ...
    async def async_split_documents(self, documents: List[Any]) -> List[Any]:
        """
        分割文档列表
    ...
    async def _optimize_chunks(self, chunks: List[str]) -> List[str]:
        ...
        for i in range(1, len(chunks)):
            ...
            # 如果相似度高于阈值，合并两个片段
            if similarity > self.similarity_threshold and len(current_chunk) + len(chunks[i]) <= self.chunk_size * self.merge_max_ratio:  # 相似度阈值
    ...
    async def _calculate_similarity(self, text1: str, text2: str) -> float:
        ...
        # 异步调用，非阻塞
        embedding1 = await asyncio.to_thread(self.embedding_model.embed_query, text1)
        embedding2 = await asyncio.to_thread(self.embedding_model.embed_query, text2)
